...
class TestSWA(TestCase):
    ...
    def test_swa(self):
        def sgd_constructor(params):
            ...
            return SWA.SWA(sgd, swa_start=1000, swa_freq=1, swa_lr=1e-3)
        ...
        def adam_constructor(params):
            ...
            return SWA.SWA(adam, swa_start=1000, swa_freq=1, swa_lr=1e-2)

        def adadelta_constructor(params):
            adadelta = optim.Adadelta(params)
            return SWA.SWA(adadelta, swa_start=1000, swa_freq=1)

        def adagrad_constructor(params):
            adagrad = optim.Adagrad(params, lr=1e-1)
            return SWA.SWA(adagrad, swa_start=1000, swa_freq=1, swa_lr=1e-2)

        def adamax_constructor(params):
            adamax = optim.Adamax(params, lr=1e-1)
            return SWA.SWA(adamax, swa_start=1000, swa_freq=1, swa_lr=1e-2)

        def rmsprop_constructor(params):
            rmsprop = optim.RMSprop(params, lr=1e-2)
            return SWA.SWA(rmsprop, swa_start=1000, swa_freq=1, swa_lr=1e-3)

        def rprop_constructor(params):
            rprop = optim.Rprop(params, lr=1e-2)
            return SWA.SWA(rprop, swa_start=1000, swa_freq=1, swa_lr=1e-3)

        def asgd_constructor(params):
            asgd = optim.ASGD(params, lr=1e-3)
            return SWA.SWA(asgd, swa_start=1000, swa_freq=1, swa_lr=1e-3)

        def lbfgs_constructor(params):
            lbfgs = optim.LBFGS(params, lr=5e-2, max_iter=5)
            return SWA.SWA(lbfgs, swa_start=1000, swa_freq=1, swa_lr=1e-3)
    ...
    def test_swa_auto(self):
        ...
        swa_start = 5
        swa_freq = 2
        opt = SWA.SWA(opt, swa_start=swa_start, swa_freq=swa_freq, swa_lr=0.001)
    ...
    def test_swa_lr(self):
        ...
        opt = SWA.SWA(opt, swa_start=swa_start, swa_freq=swa_freq, swa_lr=swa_lr)
    ...
    def test_swa_auto_mode_detection(self):
        ...
        opt = SWA.SWA(base_opt, swa_start=swa_start, swa_freq=swa_freq, swa_lr=swa_lr)
        self.assertEqual(opt._auto_mode, True)

        opt = SWA.SWA(base_opt, swa_start=swa_start, swa_freq=swa_freq)
        self.assertEqual(opt._auto_mode, True)

        with self.assertWarnsRegex("Some of swa_start, swa_freq is None"):
            opt = SWA.SWA(base_opt, swa_start=swa_start, swa_lr=swa_lr)
            self.assertEqual(opt._auto_mode, False)

        with self.assertWarnsRegex("Some of swa_start, swa_freq is None"):
            opt = SWA.SWA(base_opt, swa_freq=swa_freq, swa_lr=swa_lr)
            self.assertEqual(opt._auto_mode, False)

        with self.assertWarnsRegex("Some of swa_start, swa_freq is None"):
            opt = SWA.SWA(base_opt, swa_start=swa_start)
            self.assertEqual(opt._auto_mode, False)

        with self.assertWarnsRegex("Some of swa_start, swa_freq is None"):
            opt = SWA.SWA(base_opt, swa_freq=swa_freq)
            self.assertEqual(opt._auto_mode, False)

        with self.assertWarnsRegex("Some of swa_start, swa_freq is None"):
            opt = SWA.SWA(base_opt, swa_lr=swa_lr)
            self.assertEqual(opt._auto_mode, False)
    ...
    def test_swa_raises(self):
        ...
        with self.assertRaisesRegex(
            ...
            opt = SWA.SWA(opt, swa_start=1, swa_freq=2, swa_lr=-1e-4)

        with self.assertRaisesRegex(
                ValueError, "Invalid swa_freq: 0"):
            opt = SWA.SWA(opt, swa_start=1, swa_freq=0, swa_lr=1e-4)
    ...
    def _test_bn_update(self, data_tensor, dnn, device, label_tensor=None):
        ...
        swa = SWA.SWA(optim.SGD(dnn.parameters(), lr=1e-3))
