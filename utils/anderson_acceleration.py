...
def adaptive_anderson(X, obj_fun, lagrange=[-15, 1], eigenval_offset=0):
    ...
    if lagrange is not None:
        ...
        obj_extr = lambda lambda_val: obj_fun(anderson_precomputed(X, RR, lambda_val)[0])
        ...
        x_extr, c = anderson_precomputed(X, RR, lambda_opt)
    else:
        x_extr, c = anderson_precomputed(X, RR, 0)
